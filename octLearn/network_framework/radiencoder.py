...
from octLearn.processing.distribution import NormalLogProb
...
class RateDistortionAutoencoder(nn.Module):
    def __init__(self, latent_size, lambda0=1, lambda1=1, lambda2=1, lambda3=1):
        super().__init__()
    ...
    def forward(self, z, x, xpred, xdist):
        ...
        log_d_x = torch.log(d_x_xp + self.lambda3 * s_x_xp + EPSILON)
        ...
        loss = -self.lambda2 * log_p_z + self.lambda0 * log_d_x + self.lambda1 * d_xp_xd
